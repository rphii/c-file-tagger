...
inline void str_print_verbose(Str *str)
{
    if(str) {
        printf("ptr %p\n", str);
        printf("small.active %u | small.len %u\n", (bool)(str->small.f & STR_MASK_ACTIVE), str->small.f & STR_MASK_LEN);
        printf(".first %zu | .last %zu | .cap %zu\n", str->first, str->last, str->cap);
        printf("  %p str->s\n  %p str->small.s\n", str->s, str->small.s);
        printf("  %zu [%.100s...]\n", str_length(str), str_iter_begin(str));
    } else {
        printf("str argument pointer is (null)\n");
    }
}
...
ErrImpl str_copy(Str *dst, const Str *src) /*{{{*/
{
    ...
    TRYC(str_reserve(dst, len));
    if(STR_SMALL_ACTIVE(dst)) {
        if(len) {
            memcpy(dst->small.s, str_iter_begin(src), len);
        }
        STR_SMALL_LEN_SET(dst, str_length(src));
        dst->small.s[str_length(src)] = 0;
    } else {
        if(len) {
            memcpy(dst->s, str_iter_begin(src), len);
        }
        dst->last = str_length(src);
        dst->s[dst->last] = 0;
    }
    return 0;
error:
...
ErrImpl str_reserve(Str *str, size_t cap)
{
    ...
    if(cap < STR_SHORT_CAP) {
        ...
        size_t cap_is = str->cap; \
        if(cap > cap_is) { \
            size_t required = str->cap ? str->cap : VEC_DEFAULT_SIZE;\
            while(required < cap) required *= 2; \
            if(required > str->cap) { \
                void *temp = realloc(str->s, sizeof(*str->s) * required); \
                if(!temp) return VEC_ERROR_REALLOC; \
                str->s = temp; \
                memset(&str->s[cap_is], 0, sizeof(*str->s) * (required - cap_is)); \
                str->cap = required; \
            } \
        } \
        /* reassign values */
        if(STR_SMALL_ACTIVE(&temp)) {
            str->last = STR_SMALL_LEN(&temp);
            memcpy(str->s, temp.small.s, STR_SMALL_LEN(&temp));
        }
    }
    return 0;
error:
...
ErrImpl str_extend_back(Str *str, Str *add)
{
    ...
    size_t until = str_length(add);
    size_t require = str_alloced_bytes_in_use(str) + until; // str->last + until; //TODO:pointer-access
    size_t from = require < str_length(str) ? str_length(str) - require : 0;
    TRYC(str_reserve(str, require));
    if(STR_SMALL_ACTIVE(str)) {
        STR_SMALL_LEN_ADD(str, until);
    } else {
        str->last += until;
    }
    for(size_t i = 0; i < until; i++) {
        char value = str_get_at(add, i);
        str_set_at(str, from + i, value);
    }
    return 0;
error:
    return -1;
...
inline void str_pop_front(Str *str, char *c) /*{{{*/
{
    ASSERT_ARG(str);
    if(STR_SMALL_ACTIVE(str)) {
        char c0 = str->small.s[0];
        memmove(str->small.s, str->small.s + 1, STR_SMALL_LEN(str));
        if(c) *c = c0;
        STR_SMALL_LEN_ADD(str, -1);
    } else {
...
inline char *str_iter_begin(const Str *str)
{
    ASSERT_ARG(str);
    if(STR_SMALL_ACTIVE(str)) {
        return (char *)str->small.s;
    } else {
        //printff("ITERATE FROM BEGIN %zx", str->first);
        //for(size_t i = 0; i < STR_SHORT_CAP; ++i) {
        //    printf("%zu[%c] ", i, str->small.s[i]);
        //}
        //printff("\nFOR STRING: %s", str->s);
        return &str->s[str->first];
    }
}
...
inline char *str_iter_end(const Str *str)
{
    ASSERT_ARG(str);
    if(STR_SMALL_ACTIVE(str)) {
        return (char *)&str->small.s[STR_SMALL_LEN(str)];
    } else {
...
inline void str_triml(Str *str) //{{{
{
    ...
    if(STR_SMALL_ACTIVE(str)) {
        size_t i = 0;
        while(i < STR_SHORT_CAP && isspace(str->small.s[i])) {
            ++i;
        }
        memmove(str->small.s, str->small.s + i, sizeof(char));
        STR_SMALL_LEN_ADD(str, -i);
    } else {
...
inline void str_trimr(Str *str) //{{{
{
    ASSERT_ARG(str);
    if(STR_SMALL_ACTIVE(str)) {
        size_t i = STR_SMALL_LEN(str);
        while(i && isspace(str->small.s[i])) {
            --i;
        }
        str->small.s[i] = 0;
        STR_SMALL_LEN_CLEAR(str);
...
inline void str_clear_to_last(Str *str) { // TODO:REMOVE???
    ...
    if(STR_SMALL_ACTIVE(str)) {
        ...
        str->first = str->last; // TODO:pointer-access
    }
}
...
inline int str_fmt_va(Str *str, const char *format, va_list argp) //{{{
{
    ...
    //size_t len_new = str->last + len_app; //TODO:pointer-access
    size_t len_new = str_alloced_bytes_in_use(str) + len_app; //TODO:pointer-access
    if(str_reserve(str, len_new)) {
        THROW("failed reserving %zu bytes!", len_new);
    }
    // actual append
    int len_chng = vsnprintf(str_iter_end(str), len_app + 1, format, argp);
    // check for success
    if(len_chng >= 0 && (size_t)len_chng <= len_app) {
        if(STR_SMALL_ACTIVE(str)) {
            STR_SMALL_LEN_ADD(str, len_chng);
        } else {
...
inline Str str_splice(Str *to_splice, Str *prev_splice, char sep) {/*{{{*/
    ...
    //if(prev_splice && prev_splice->s) { // TODO:accessing s
